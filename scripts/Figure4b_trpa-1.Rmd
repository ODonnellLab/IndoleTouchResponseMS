...
dataframe <-TouchResponses %>%
  subset(food =='OP50' & is.na(additive) & !time_off_food_min %in% c("10", "0", "30") 
         & genotype %in% c("N2", "cest-1.2", "glo-1", "trpa-1") 
         & date > "2023-03-01" )

dataframe$genotype <- factor(dataframe$genotype, levels = c("N2", "cest-1.2", "glo-1", "trpa-1"),
                             ordered = TRUE)
...
plot <- dataframe %>%
  ...
  geom_signif(comparisons = list(c("N2", "cest-1.2"), c("N2", "glo-1"),
                                 c("N2", "trpa-1")),
