---
title: "Anterior touch response, gut vs AWC"
author: "Julia Balch"
date: "2023-04-16"
output: html_document
---
plot of AWC rescue anterior touch data
...
dataframe <-TouchResponses %>%
  subset(food =='OP50' & is.na(additive) & !time_off_food_min %in% c("10", "0", "30") 
         & genotype %in% c("N2", "cest-1.2", "cest-1.2p::cest-1.2","ges-1p::cest-1.2") 
         & date > "2023-03-01" )

dataframe$genotype <- factor(dataframe$genotype, levels = c("N2", "cest-1.2","ges-1p::cest-1.2", "cest-1.2p::cest-1.2"),
                             ordered = TRUE)
...
plot <- dataframe %>%
  ...
  scale_fill_manual(values = c("N2" = "#582878",
                               ...
                               "ges-1p::cest-1.2" = "#31678e")) +
  labs(y = "Body Bends per Reversal (n)")+
  geom_signif(comparisons = list(c("N2", "cest-1.2"), c("N2", "ges-1p::cest-1.2"),
                                 c("N2", "cest-1.2p::cest-1.2")),
              map_signif_level = TRUE, tip_length = 0.015, y_position = c(3.25, 3.65, 4)) +
